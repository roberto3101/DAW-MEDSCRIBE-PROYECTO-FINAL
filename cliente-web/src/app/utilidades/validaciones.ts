...
export function sanitizarDocumento(tipo: string, valor: string): string {
  const v = valor || '';
  if (tipo === 'DNI') return v.replace(/\D/g, '').slice(0, 8);
  if (tipo === 'CE') return v.replace(/\D/g, '').slice(0, 12);
  if (tipo === 'Pasaporte') return v.replace(/[^A-Za-z0-9]/g, '').toUpperCase().slice(0, 12);
  return v;
}
...
export function validarTelefono(valor: string): ResultadoValidacion {
  ...
  // Validacion simple: digitos, signos + - ( ) y espacios, entre 7 y 20 chars totales.
  const regexTelefonoInternacional = /^[+]?[\d\s\-()]{7,20}$/;
  if (!regexTelefonoInternacional.test(v)) return 'Numero de telefono invalido';
  return null;
}
...
export function validarRuc(valor: string): ResultadoValidacion {
  const v = (valor || '').trim();
  if (!v) return 'El RUC es obligatorio';
  if (!/^\d{11}$/.test(v)) return 'El RUC debe tener 11 digitos';
  return null;
}
