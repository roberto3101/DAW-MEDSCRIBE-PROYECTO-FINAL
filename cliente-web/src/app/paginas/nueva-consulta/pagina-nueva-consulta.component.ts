          ...
          <div class="bg-white rounded-xl border border-slate-200 p-6">
            <div class="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <select [ngModel]="tipoDocumento()" (ngModelChange)="tipoDocumento.set($event)" [disabled]="estadoFlujo() !== 'esperando'"
              ...
              <div>
                ...
                <select [ngModel]="especialidad()" (ngModelChange)="especialidad.set($event)" [disabled]="estadoFlujo() !== 'esperando'"
                  class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-medico-500/20 disabled:opacity-50">
            ...
            <div *ngIf="diarizacionActiva()" class="flex gap-2 mb-4">
              <div class="flex-1 text-xs font-medium px-3 py-2 rounded-lg border bg-medico-50 border-medico-300 text-medico-700 text-center">
...
export class PaginaNuevaConsultaComponent {
  ...
  private static readonly CLAVE_PERSISTENCIA = 'medscribe_nueva_consulta_v1';

  diarizacionActiva = signal(false);
  motorDiarizacion = signal<'deepgram'>('deepgram');
  infoDiarizacion = signal<any>(null);

  estadoFlujo = signal<EstadoFlujo>('esperando');
  tipoDocumento = signal<string>('SOAP');
  especialidad = signal<string>('Medicina General');
  ...
  private hidratandoEstado = false;

  constructor() {
    this.restaurarEstadoPersistido();
    ...
    effect(() => {
      const snapshot = {
        ...
        notaClinica: this.notaClinica(),
        infoDiarizacion: this.infoDiarizacion(),
        diarizacionActiva: this.diarizacionActiva(),
        estadoFlujo: this.estadoFlujo(),
      };
      if (this.hidratandoEstado) return;
      try {
        const estadoSerializable: EstadoFlujo =
          snapshot.estadoFlujo === 'completado' ? 'completado' :
          (snapshot.transcripcion || snapshot.notaClinica) ? 'completado' : 'esperando';
        localStorage.setItem(
          PaginaNuevaConsultaComponent.CLAVE_PERSISTENCIA,
          JSON.stringify({ ...snapshot, estadoFlujo: estadoSerializable })
        );
      } catch { /* localStorage lleno o deshabilitado */ }
    });
  }

  private restaurarEstadoPersistido(): void {
    try {
      const crudo = localStorage.getItem(PaginaNuevaConsultaComponent.CLAVE_PERSISTENCIA);
      if (!crudo) return;
      const datos = JSON.parse(crudo);
      this.hidratandoEstado = true;
      if (datos.paciente) this.pacienteEncontrado.set(datos.paciente);
      if (typeof datos.textoBusqueda === 'string') this.textoBusqueda.set(datos.textoBusqueda);
      if (typeof datos.tipoDocumento === 'string') this.tipoDocumento.set(datos.tipoDocumento);
      if (typeof datos.especialidad === 'string') this.especialidad.set(datos.especialidad);
      if (typeof datos.transcripcion === 'string') this.transcripcion.set(datos.transcripcion);
      if (typeof datos.notaClinica === 'string') this.notaClinica.set(datos.notaClinica);
      if (datos.infoDiarizacion) this.infoDiarizacion.set(datos.infoDiarizacion);
      if (typeof datos.diarizacionActiva === 'boolean') this.diarizacionActiva.set(datos.diarizacionActiva);
      const flujo: EstadoFlujo = datos.estadoFlujo === 'completado' ? 'completado' : 'esperando';
      this.estadoFlujo.set(flujo);
      setTimeout(() => { this.hidratandoEstado = false; }, 0);
    } catch {
      this.hidratandoEstado = false;
    }
  }

  private limpiarEstadoPersistido(): void {
    try {
      localStorage.removeItem(PaginaNuevaConsultaComponent.CLAVE_PERSISTENCIA);
    } catch { /* ignore */ }
  }
