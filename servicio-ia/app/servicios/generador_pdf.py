...
def _firmas(s, cfg, fmt):
    ...
    firma_m = _base64_a_ruta_temporal(cfg.get("firma_medico", "") or "")
    firma_c = _base64_a_ruta_temporal(cfg.get("firma_clinica", "") or "")
    nombre_m = cfg.get("nombre_medico", "") or ""
    col_m = cfg.get("colegiatura", "") or ""
    esp_m = cfg.get("especialidad_medico", "") or ""
    nombre_cl = cfg.get("nombre_clinica", "") or ""
    ...
    if firma_m:
        try:
            ...
            pass
    if nombre_m or firma_m:
        if not firma_m:
            izq.append(Paragraph("_" * 28, s["FirmaNombre"]))
        izq.append(Paragraph(f"<b>{nombre_m or 'Medico Tratante'}</b>", s["FirmaNombre"]))
        if col_m:
            izq.append(Paragraph(f"CMP: {col_m}", s["FirmaDetalle"]))
        if esp_m:
            izq.append(Paragraph(esp_m, s["FirmaDetalle"]))
        if not nombre_m and not col_m and not esp_m:
            izq.append(Paragraph("Firma del Medico", s["FirmaDetalle"]))
    ...
    if firma_c:
        try:
            ...
            pass
    if nombre_cl or firma_c:
        if not firma_c:
            der.append(Paragraph("_" * 28, s["FirmaNombre"]))
        der.append(Paragraph(f"<b>{nombre_cl or 'Clinica'}</b>", s["FirmaNombre"]))
