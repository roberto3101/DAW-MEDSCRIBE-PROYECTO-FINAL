...
def _firma_base64_a_archivo_temporal(data_url: str) -> str:
    """Convierte un data URL (data:image/png;base64,...) a un archivo temporal y devuelve la ruta."""
    if not data_url or not data_url.startswith("data:image"):
        return ""
    try:
        cabecera, datos = data_url.split(",", 1)
        extension = "png"
        if "image/jpeg" in cabecera or "image/jpg" in cabecera:
            extension = "jpg"
        elif "image/webp" in cabecera:
            extension = "webp"
        ruta = tempfile.NamedTemporaryFile(suffix=f".{extension}", delete=False).name
...
def _agregar_firmas(doc: Document, config: dict) -> None:
    ...
    nombre_medico = config.get("nombre_medico", "") or ""
    colegiatura = config.get("colegiatura", "") or ""
    especialidad = config.get("especialidad_medico", "") or ""
    nombre_clinica = config.get("nombre_clinica", "") or ""
    firma_medico_b64 = config.get("firma_medico", "") or ""
    firma_clinica_b64 = config.get("firma_clinica", "") or ""

    ruta_firma_medico = _firma_base64_a_archivo_temporal(firma_medico_b64)
    ruta_firma_clinica = _firma_base64_a_archivo_temporal(firma_clinica_b64)

    columna_medico_visible = bool(nombre_medico or ruta_firma_medico)
    columna_clinica_visible = bool(nombre_clinica or ruta_firma_clinica)

    if not columna_medico_visible and not columna_clinica_visible:
        return
    ...
    try:
        ...
        if columna_medico_visible:
            celda_firma_m = tabla.cell(0, 0).paragraphs[0]
            celda_firma_m.alignment = WD_ALIGN_PARAGRAPH.CENTER
            if ruta_firma_medico:
                archivos_temporales.append(ruta_firma_medico)
                try:
                    celda_firma_m.add_run().add_picture(ruta_firma_medico, width=Cm(4), height=Cm(1.5))
                except Exception:
                    run = celda_firma_m.add_run("_" * 30)
            ...
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = p.add_run(nombre_medico or "Medico Tratante")
            run.font.bold = True
            run.font.size = Pt(9)
            ...
            if not nombre_medico and not colegiatura and not especialidad:
                p = tabla.cell(2, 0).paragraphs[0]
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                run = p.add_run("Firma del Medico")
                run.font.size = Pt(8)
                run.font.color.rgb = TEXTO_SUAVE

        if columna_clinica_visible:
            celda_firma_c = tabla.cell(0, 1).paragraphs[0]
            celda_firma_c.alignment = WD_ALIGN_PARAGRAPH.CENTER
            if ruta_firma_clinica:
                archivos_temporales.append(ruta_firma_clinica)
                try:
                    celda_firma_c.add_run().add_picture(ruta_firma_clinica, width=Cm(4), height=Cm(1.5))
                except Exception:
                    run = celda_firma_c.add_run("_" * 30)
                    run.font.color.rgb = TEXTO_CLARO
                    run.font.size = Pt(8)
            else:
                run = celda_firma_c.add_run("_" * 30)
            ...
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = p.add_run(nombre_clinica or "Clinica")
            run.font.bold = True
            run.font.size = Pt(9)

            p = tabla.cell(2, 1).paragraphs[0]
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = p.add_run("Sello Institucional")
            run.font.size = Pt(8)
            run.font.color.rgb = TEXTO_SUAVE
    finally:
